...
1. Orient: call \`pack_context\` with a \`query\` describing the task in plain language — one call returns the most relevant symbols (path:line — kind name — signature) plus a skeleton of the top files. (\`get_project_pulse\`/\`get_repo_map\` give a high-level overview.)
